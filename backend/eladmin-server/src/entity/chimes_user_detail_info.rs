...
impl ChimesUserDetailInfo {
    ...
    pub fn to_user(&self) -> ChimesUserInfo {
        let self_dept_id = match self.dept.clone() {
            Some(np) => np.dept_id.clone(),
            None => self.dept_id.clone(),
        };
        ChimesUserInfo {
            ...
            Ok(ts) => {
                match ts {
                    Some(mp) => {
                        ...
                        selfmp.dept = match ChimesDeptInfo::from_id(rb, &selfmp.dept_id.clone().unwrap_or_default()).await {
                            Ok(lst) => {
                                lst
                            }
                            Err(_) => {
    ...
    pub async fn save(&self,rb: &Rbatis) -> Result<bool, Error> {
        ...
        if ret.is_none() {
            ...
            rm_user_role_info.user_id = self.user_id;
        ...
        for row in self.roles.clone() {
            log::info!("Insert new roles: {}", row.role_id.clone().unwrap_or_default());
        ...
        if ret.is_none() {
            let mut rm_user_job_info = ChimesUserJobInfo::default();
            rm_user_job_info.user_id = self.user_id;
            ret = match rm_user_job_info.remove_batch(rb).await {
                Ok(_) => {
                    None
                }
                Err(err) => {
                    log::info!("Remove user_job_info occurred an error {}", err);
                    Some(err)
                }
            };
        }
        for row in self.jobs.clone() {
            let mut svrow_user_job_info = ChimesUserJobInfo::default();
            svrow_user_job_info.user_id = self.user_id.clone();
            svrow_user_job_info.job_id = row.job_id.clone();
            ret = match svrow_user_job_info.save(rb).await {
                Ok(_) => {
                    None
                }
                Err(err) => {
                    log::info!("Save user_job_info occurred an error {}", err);
                    Some(err)
                }
            };
        }
