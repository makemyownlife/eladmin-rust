        ...
        >
          ...
          >
            <el-tooltip
              ...
              placement="top"
            >
              <span class="role-span">功能分配</span>
            </el-tooltip>
            <el-button
              v-permission="['admin','roles:edit']"
              :disabled="!showButton"
...
import { getPermissionTree, querySubPermissions, getPermissionChildren } from '@/api/system/permission'
...
export default {
  ...
  cruds() {
    return CRUD({ title: '角色', url: 'api/v1/role/paged', sort: 'level,asc', idField: 'role_id', crudMethod: { ...crudRoles }})
  },
  ...
  data() {
    return {
      ...
      permissions: [], permissionIds: [], permissionLoading: false,
      permission: {
        add: ['admin', 'roles:add'],
  ...
  methods: {
    ...
    handleCurrentChange(val) {
      ...
      if (val) {
        ...
        this.menuIds = []
        this.permissionIds = []
        get(val.role_id).then(res => {
          val.menus = res.data.menus
          val.permissions = res.data.permissions
          var nemus = []
          var perms = []
          val.menus.forEach(function(data) {
            nemus.push(data.menu_id)
          })
          val.permissions.forEach(function(data) {
            perms.push(data.id)
          })
          _this.menuIds = nemus
          _this.permissionIds = perms
        })
        // 初始化默认选中的key
      ...
      getChild(menu.id).then(res => {
        ...
        if (this.menuIds.indexOf(menu.id) !== -1) {
          for (let i = 0; i < res.data.length; i++) {
            const index = this.menuIds.indexOf(res.data[i])
            if (index !== -1) {
    ...
    saveMenu() {
      ...
      const role = { role_id: this.currentId, menus: [], permissions: [] }
      // 得到已选中的 key 值
      this.menuIds.forEach(function(id) {
        const menu = { menu_id: id }
        role.menus.push(menu)
      })
      this.permissionIds.forEach(function(id) {
        const pm = { id: id }
        role.permissions.push(pm)
      })
    ...
    update() {
      ...
      crudRoles.get(this.currentId).then(res => {
        for (let i = 0; i < this.crud.data.length; i++) {
          if (res.role_id === this.crud.data[i].role_id) {
    ...
    getPermissions() {
      getPermissionTree().then(res => {
        this.permissions = res.data.map(ch => {
          ch.children = null
          return ch
        })
      })
    },
    getPermissionData(node, resolve) {
      setTimeout(() => {
        querySubPermissions(node.data.id ? node.data.id : 0).then(res => {
          resolve(res.data)
        })
      }, 100)
    },
    percheckall() { // 功能el-tree全选事件
      if (this.percheckbox) { // 全选
        this.$refs.permission.setCheckedNodes(this.permissions)
      } else { // 取消选中
        this.$refs.permission.setCheckedKeys([])
      }
    },
    permissionChange(perm) {
      // 获取该节点的所有子节点，id 包含自身
      getPermissionChildren(perm.id).then(res => {
        // 判断是否在 menuIds 中，如果存在则删除，否则添加
        if (this.permissionIds.indexOf(perm.id) !== -1) {
          for (let i = 0; i < res.data.length; i++) {
            const index = this.permissionIds.indexOf(res.data[i])
            if (index !== -1) {
          ...
          for (let i = 0; i < res.data.length; i++) {
            const index = this.permissionIds.indexOf(res.data[i])
            if (index === -1) {
              this.permissionIds.push(res.data[i])
            }
          }
        }
        this.$refs.permission.setCheckedKeys(this.permissionIds)
      })
    }
  }
}
