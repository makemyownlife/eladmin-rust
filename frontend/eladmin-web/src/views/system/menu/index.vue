    ...
    <el-dialog append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="580px">
      <el-form ref="form" :inline="true" :model="form" :rules="rules" size="small" label-width="80px">
        ...
        <el-form-item v-show="form.type.toString() !== '0'" label="权限标识" prop="permission">
          <el-input v-model="form.permission" :disabled="form.i_frame.toString() === 'true'" placeholder="权限标识" style="width: 178px;" />
        </el-form-item>
        ...
        <el-form-item v-show="form.i_frame.toString() !== 'true' && form.type.toString() === '1'" label="组件名称" prop="name">
          <el-input v-model="form.name" style="width: 178px;" placeholder="匹配组件内Name字段" />
        </el-form-item>
        <el-form-item v-show="form.i_frame.toString() !== 'true' && form.type.toString() === '1'" label="组件路径" prop="component">
...
// crud交由presenter持有
const defaultForm = { id: null, title: null, menu_sort: 999, path: null, component: null, name: null, i_frame: false, roles: [], pid: 0, icon: null, cache: false, hidden: false, type: 0, permission: null }
export default {
  name: 'Menu',
  ...
  methods: {
    ...
    buildMenuTree(depts) {
      depts.forEach(data => {
        if (data.children) {
          this.buildMenuTree(data.children)
        }
    ...
    loadMenus({ action, parentNode, callback }) {
      if (action === LOAD_CHILDREN_OPTIONS) {
        callback()
        // crudMenu.getMenuSuperior(parentNode.id).then(res => {
