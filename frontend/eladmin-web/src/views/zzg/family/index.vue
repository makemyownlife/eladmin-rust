    ...
    <el-dialog append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="500px">
      <el-form ref="form" inline :model="form" :rules="rules" size="small" label-width="80px">
        ...
        <el-form-item label="顶级部门">
          <el-radio-group v-model="form.is_top" style="width: 140px">
        ...
        <el-form-item v-if="form.is_top === '0'" style="margin-bottom: 0;" label="上级部门" prop="pid">
          <treeselect
            v-model="form.pid"
            :load-options="loadDepts"
            :tree-props="{children: 'children', hasChildren: 'has_children', id: 'dept_id'}"
            :options="depts"
            style="width: 370px;"
            placeholder="选择上级类目"
          />
      ...
      <el-table-column v-if="checkPer(['admin','dept:edit','dept:del'])" label="操作" width="130px" align="center" fixed="right">
        <template slot-scope="scope">
          <udOperation
            ...
            :disabled-dle="scope.row.dept_id === 1"
...
const defaultForm = { dept_id: null, name: null, is_top: '1', sub_count: 0, pid: null, dept_sort: 999, enabled: 'true' }
export default {
  name: 'Dept',
  components: { Treeselect, crudOperation, rrOperation, udOperation, DateRangePicker },
  cruds() {
    return CRUD({ title: '部门', url: 'api/v1/dept/paged', idField: 'id', debug: true, crudMethod: { ...crudDept }})
  },
  ...
  methods: {
    ...
    getDepts(tree) {
      console.log('GetDepts: Tree', tree)
      crudDept.getDeptSuperior([0]).then(res => {
    ...
    loadDepts({ action, parentNode, callback }) {
      console.log('Tree Option: ', action)
      if (action === LOAD_CHILDREN_OPTIONS) {
        crudDept.getDepts({ enabled: true, pid: parentNode.dept_id }).then(res => {
